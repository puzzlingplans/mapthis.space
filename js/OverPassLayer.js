  ...
  options: {
    ...
    callback: function(data) {
      ...
      for(var i = 0; i < data.elements.length; i++) {
        ...
        //console.log(i + "\t" + e.tags['name']);
        if (e.id in this.instance._ids)
          continue; // don't add duplicates
        this.instance._ids[e.id] = e;
        var pos;
        if (e.type == "node") {
          pos = new L.LatLng(e.lat, e.lon);
        } else if (e.type == "way") {
          var cenlat = (e.bounds.minlat + e.bounds.maxlat)/2;
          var cenlon = (e.bounds.minlon + e.bounds.maxlon)/2;
          pos = new L.LatLng(cenlat,cenlon);
        }
        var popup = this.instance._poiInfo(e);
        var circle = L.circle(pos, 5, {
          color: '#337733',
          fillColor: '#66ff66',
          fillOpacity: 0.5
        })
        ...
        this.instance._circles[e.id] = circle;
      }
    },
    ...
    minZoomIndicatorOptions: {
      ...
      minZoomMessage: "POI update suspended; zoom in to resume"
    },
  },
